...
%gravità e di velocità, uso solo le perturbazioni per eccitare il sistema
it=1:60;
...
figure, plot(it,mcal(1,:),'r-'),hold on,plot(it,mcal(2,:),'g-'),title('Parametri del modello invertiti: gravità(r) velocità(v)');
...
figure, plot(it,dn(1,:),'r-'),hold on,plot(it,dn(2,:),'g-'),title('Perturbazioni dei dati: tempo(r) spazio(g)');
figure, plot(it,mcalr(1,:),'r-'),hold on,plot(it,mcalr(2,:),'g-'),hold on,plot(it,mcalr(3,:),'b-'),title('Parametri del modello invertiti: gravità(r) velocità(v) attrito(n)');
