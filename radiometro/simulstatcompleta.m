...
temp=0;
hi=linspace(90,30,15);
for h=1:length(hi) %angoli di puntamento ai quali effettuo le misurazioni
    temp=[temp; hi(h)];
end
...
cov_dati=diag(cov(dn')); %covarianza del rumore perchè cov toglie il valore medio sulle realizzazioni
%il rumore sulle singole componenti dei dati è la radice di cov_dati (ho preso solo i valori diagonali della matrice di covarianza, perchè di suppone
%che il vettore del rumore abbia componenti incorrelate), la deviazione
%standard nei dati sarà il rapporto tra deviaz standard del rumore e valore
%medio dei dati
...
%è immediato verificare che cov_mod=Gp*cov_dati*Gp'
disp('La covarianza del modello completa è: ')
disp(cov(mric'))
disp('La covarianza del modello calcolata attraverso la conoscenza della covarianza dei dati è: ')
disp(Gp*(cov(dn')*Gp'))

%analisi via SVD conferma che la componente meno influenzata dal rumore è
...
for i=1:100
    ...
    mric(:,i)=mric(:,i)+V(:,1)*(1/D(1,1))*U(:,1)'*dn(:,i)+V(:,2)*(1/D(2,2))*U(:,2)'*dn(:,i);
